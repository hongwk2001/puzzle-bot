...
def _save(pieces, out_directory):
    # Using a dictionary to store the best match for each pair of sides,
    # ensuring that we only keep the one with the minimum error.
    # The key is a frozenset of tuples representing the two sides being matched,
    # which makes the key order-independent.
    unique_matches = {}
    for piece_id, piece in pieces.items():
        for side_idx, side_fits in enumerate(piece.fits):
            for matched_piece_id, matched_side_idx, error in side_fits:
                side1 = (piece_id, side_idx)
                side2 = (matched_piece_id, matched_side_idx)

                # Create a canonical key for the pair of sides
                match_key = frozenset([side1, side2])

                # If we haven't seen this pair, or if the new error is lower, store/update it.
                if match_key not in unique_matches or error < unique_matches[match_key][2]:
                    unique_matches[match_key] = (list(side1), list(side2), error)

    # Sort the unique matches by error
    sorted_matches = sorted(unique_matches.values(), key=lambda x: x[2])
    path = os.path.join(out_directory, 'sorted_matches.json')
    with open(path, 'w') as f:
        json.dump(sorted_matches, f) # Using indent for better readability
    ...
    for si, side in enumerate(piece.sides):
        ...
        for other_piece_id, other_piece in ps.items():
            ...
            for sj, other_side in enumerate(other_piece.sides):
                ...
                part_of_solution = ([(piece_id, si), (other_piece_id, sj)] in SOLUTION) or (
                            [(other_piece_id, sj), (piece_id, si)] in SOLUTION)

                # compute the error between our piece's side and this other piece's side
                error = side.error_when_fit_with(other_side, render=part_of_solution or debug,
                                                 debug_str=f'{piece_id}[{si}] vs {other_piece_id}[{sj}]')
        ...
        if debug:
            ...
            if len(piece.fits[si]) > nth:
                ...
                print(
                    f"\t1st match error: {least_error} \t ==> {nth}th match error: {nth_match_error} \t ==> ratio: {nth_match_error / least_error}")
